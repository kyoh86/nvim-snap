...
local function build_action(c)
  if c.kind ~= "regression" then
    return nil, "case kind must be regression"
  end
...
local function confirm_actions(actions)
  local message = string.format("Accept actual snapshots for %d case(s)? [y/N]: ", #actions)
...
function M.run(args_list)
  ...
  for _, action in ipairs(actions) do
    ...
    if not ok then
      util.err_write(action.case.name .. ": " .. (err or "failed to accept expected"))
      failed = true
    else
      print(action.case.name .. "\taccepted")
    end
  end
